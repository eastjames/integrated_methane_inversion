...
if "$WriteBCs"; then
    ...
    #sbatch -W -p seas_compute -t 2-00:00 --mem 64000 -o logs/slurm-%j.out -c 1 --wrap "module load python; conda activate $CondaEnv; echo `which python`; python write_tropomi_GC_daily_avgs.py" #; wait;
    sbatch -p huce_intel,test,seas_compute -t 04:00 --mem 64000 -o logs/slurm-%j.out -c 1 --wrap "module load python; conda activate $CondaEnv; echo `which python`; python write_tropomi_GC_daily_avgs.py" #; wait;
    #sbatch -W -p seas_compute -t 2-00:00 --mem 64000 -o logs/slurm-%j.out       --wrap "module load python; conda activate $CondaEnv; python calculate_bias.py"; wait;
    #sbatch -W -p seas_compute -t 2-00:00 --mem 64000 -o logs/slurm-%j.out       --wrap "module load python; conda activate $CondaEnv; python write_boundary.py"; wait;
